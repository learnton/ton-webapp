...
export function vcEncrypt(
  ...
  const encoded = u8aConcat(
    u8aFixLength(
      numberToU8a(DEFAULT_VC_ENCRYPT_VERSION),
      VERSION_BYTE_LENGTH * 8
    ),
...
export function vcDecrypt(
  ...
  const typeU8a = decoded.subarray(
    VERSION_BYTE_LENGTH,
    VERSION_BYTE_LENGTH + VC_TYPE_BYTE_LENGTH
  );
  const contentU8a = decoded.subarray(
    VERSION_BYTE_LENGTH + VC_TYPE_BYTE_LENGTH
  );
